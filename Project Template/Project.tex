...
\puttitle{
	...
	name1=Edoardo Silvio Gribaldo, % Author Name and Surname
...
\chapter{Dataset}
The choice of the dataset was not really easy. I chose this dataset because is a medium size dataset. Indeed the free version of  Elasticsearch allowed me only to upload files of maximum 50 MB. Moreover the .csv format made it simple to explore and clean a little bit the data with a simple script in Python. Moreover there were a lot of datasets with reviews that had only 2 columns (the review and the sentiment), clearly used to train models to predict the sentiment of a review. Of course no such dataset would have been useful for my purpose. Indeed I needed about a dozen of different columns to be able to perform 10 valuable queries. 
...
\begin{enumerate}
    ...
    \item Group the reviews by their seat type and find their average, sorting the results by ascending average
    ...
    \item During which month people flew the most in 2022?
    ...
    \item Get the 10 worst airlines for average total rating calculated only on airlines that have at least 20 reviews and whose score is not null (-1)
